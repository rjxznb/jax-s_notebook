...
光栅化：光栅可以理解为由显示屏上一个个发光点组成，在电脑上创建的三维物体或场景，需要显示在屏幕上，而想办法将三维物体显示在屏幕一个个发光点上的过程，就叫对三维物体进行光栅化；

光栅化是怎么实现的呢？**核心**一个词就是**投影：投影就是将三维物体上的某个采样点的颜色映射到二维数组的元素上，三维物体上的颜色是怎么来的呢？就是通过着色，着色需要在投影前完成；**那投影是怎么做的呢？是通过投影变换矩阵实现的；

<img src="F:\VS\jax-s_notebook\笔记图片\b89da85fbb0670f2a7e471c5ed21f892.png" style="zoom:50%;" />

我们创建的三维物体是连续的，而屏幕通过像素显示（采样）的内容总是离散的：

<img src="F:\VS\jax-s_notebook\笔记图片\cc83fbdfabfcc1095a443aac8a09ea7a.png" style="zoom:50%;" /><img src="F:\VS\jax-s_notebook\笔记图片\cc83fbdfabfcc1095a443aac8a09ea7a.png" style="zoom:50%;" />

“离散”的屏幕无法直接显示“连续”的三维物体，因此需要先对连续的物体进行“离散化”，离散化就是对三维物体的采样点进行**投影**，之后**将结果存到帧缓存上(存储一帧画面的像素值)，**再将帧缓存交给显卡，显示一副图像；

<img src="F:\VS\jax-s_notebook\笔记图片\fa430dd99c685c83e2fa15670effc213.png" style="zoom:50%;" />

光线追踪：英伟达20系显卡RTX技术的推出后，将普及光线追踪实时应用变为可能；

<img src="F:\VS\jax-s_notebook\笔记图片\79bfe5fa6ea10411b1947df953e6cfa0.jpeg" style="zoom:33%;" />

光线追踪就是想模拟下图的情景，**追踪每条光线的传播行为，计算每条光线对我们人眼观察的贡献值，即颜色值；**

![](F:\VS\jax-s_notebook\笔记图片\8aecb3de0461b474235bab730cd2b90c.png)

场景中有灯光，太阳光等许多光源，而且每个光源可以发射无数条光线，要是每条都追踪的话，先不说计算量的问题，就连怎么用代码去实现这个模拟的过程都让人觉得头痛。实际上，现实世界中存在那么多光线，真正帮助我们眼睛里成像的只有射入我们眼睛的那部分光线（上图的红色光线），而没有进入我们眼睛的光线（上图的黑色光线），则是没有用的。

因此，**我们要模拟的只需要模拟那些进入我们眼睛的光线**。考虑到**光路是可逆的**，也就是说从光源到我们人眼的这条光线的光路，不管是正着走还是逆着走，这条光线都是一样的，所以我们模拟光线传播过程的一个很巧妙的设计就是利用了光路的可逆性，**将光线的起点改为从我们人眼出发，去寻找场景中的光源，**这样连起来就是一条从连接了人眼和光源的光路，是一条有用的光线：

<img src="F:\VS\jax-s_notebook\笔记图片\64462cf234297e8a476cdb9e0da6eaec.png" style="zoom: 50%;" /><img src="F:\VS\jax-s_notebook\笔记图片\0a04c78be7218c76dde73535459a2b0e.png" style="zoom:50%;" />

光线的传播行为大致分为直射、反射和折射，而其中我们最重要模拟的就是光线的反射；

**怎么用光线追踪来做渲染呢？**

实际上光追渲染的过程就是将以上模拟过程中出现的东西，准确表达出来。首先是我们的眼睛，就定义为一个虚拟摄像机，在三维坐标系中有位置坐标，镜头朝向的向量，这些都是可以我们自己给具体参数，然后我们看到的画面就是假设是一个800x600的数组，我们要做的就是为每个数组元素填上像素颜色值，也为其定义相对我们摄像机的坐标位置，相当于我们摄像机移动的话，画幅也会跟着移动（这里涉及世界坐标系和摄像机(局部)坐标系的知识）。模拟光线的话，我们就从我们的眼睛出发，穿过800x600中的每个像素点，发射光线进入场景中，起点就是我们摄像机的位置，目标就是每个像素点对应的坐标，然后用目标坐标减起点坐标就是这条光线的方向。追踪光线可以以我们上面说明的镜面反射为例，追踪我们发射出的光线能不能最终到达光源，光源在场景中可以是一个也有它的坐标位置，光线能够到达光源是指光线与光源发生了碰撞。如果能到达光源就计算到达光源后的颜色，之后着色；如果不能到达光源就计算无法到达光源的颜色；
![](F:\VS\jax-s_notebook\笔记图片\5dd4fa673eddf4e9630074fd85cbc98f.png)

这里插一句，根据上图来看，光线要么射空，要么反射，但是光线不能碰到物体都永无止境地反射下去，因为这样是算不完的，我们可以自己设定一个阈值，就是光线反射次数超过3次还没命中光源的话，我们就判断它没法找到光源，停止继续反射。

光追对比光栅化：

- 光线追踪自然有它的优势，因为它就是模仿真实光线的传播行为，因此**给很多光影效果的模拟提供巨大的方便，**而要用光栅渲染去实现同样的阴影效果，碍于成像方法的不同，该计算过程会变得非常复杂，难以实现；
- 但是光追计算量大，一个游戏场景有成百万上千万的三角形mesh，计算光线与谁碰撞需要遍历全部三角形（采用加速结构之后仅需要遍历相交的bounding volume内的物体的三角形）， 但是现有游戏画面分辨率少说也1980x1080了吧，而且实际上穿过每个像素点一般可不只是发射一条光线，每条光线在场景中也不只是仅反射3次那么少，这些计算量一叠加上来，将使得一帧画面的计算也变得非常缓慢，即使是RTX30系列，在游戏里也做不到全画面全场景去做光线追踪渲染（现在游戏支持的光线追踪渲染都不算是完全体的光线追踪，通常是结合了光栅渲染的）；

- **光追常用于离线渲染领域** ：在电影、广告等对画质要求极高且对渲染时间要求不严格的离线渲染场景中被广泛应用。例如，电影中的特效制作团队可以花费数小时甚至数天来渲染一帧图像，以获得极其逼真的视觉效果。

光追的三个前提假设：

<img src="F:\VS\jax-s_notebook\笔记图片\微信图片_20250613105000.png" style="zoom:15%;" />

下面给出真正的光追做法；

### 5.1 whitted style ray tracing

#### 5.1.1 Ray Intersection

首先要做的就是求出光线/摄像机和物体的交点，之后才能着色/反射；

我们先拿**光线（虽然光追是逆向的，也就是从摄像机到光源这样逆向地反射，但是反射的机制和正向没有任何区别）**打到一个球体上是如何进行 whitted style 光追的举例：

如上图，光线的公式可以表示为**从光源/摄像机 o为起点，d向量为方向，r(t)表示传播时间t之后光射线到达的坐标位置的公式，**那么当一条光线打到一个球体表面上，我们需要先**计算交点p** 之后才能计算该点的法线，出射角等，计算方法就是**该点同时满足光线方程和球形表面方程，**将两个公式合并之后，我们可以发现**未知量就只有传播时间 t，**也就是**从光源/摄像机为起点需要传播多久才能打到物体表面，**这个直接解方程就能求出时间t，之后**将t带回光线方程就直接能够求接出交点坐标p；**

<img src="F:\VS\jax-s_notebook\笔记图片\微信图片_20250613105022.png" style="zoom:15%;" />

##### 5.1.1.1 Ray Intersection With lmplicit Surface

几何有显式和隐式两种表征方式，上面解方程的方法其实是对于隐式表示方式的求解方法；

<img src="F:\VS\jax-s_notebook\笔记图片\微信图片_20250613105031.png" style="zoom:15%;" />

##### 5.1.1.2 Ray lntersection With Triangle Mesh

下面介绍一下显式表面，最简单的方法就是**遍历所有三角形，求出所有和光线有交点的三角形，然后选择深度最浅的那个；**

<img src="F:\VS\jax-s_notebook\笔记图片\微信图片_20250613105035.png" style="zoom:15%;" />

具体的做法如下所示：每一个**三角形mesh都是一个平面，**我们**将他转换为一个平面方程，**之后就可以通过**光源和隐式表征几何求交点的方式求解交点，**之后再判断**该点是否在三角形内部，**如果在就有交点；

<img src="F:\VS\jax-s_notebook\笔记图片\微信图片_20250613105039.png" style="zoom:15%;" />

<img src="F:\VS\jax-s_notebook\笔记图片\微信图片_20250613105050.png" style="zoom:15%;" />

还有一种通过重心坐标直接计算光线和三角形交点的方法

<img src="F:\VS\jax-s_notebook\笔记图片\微信图片_20250613105056.png" style="zoom:15%;" />
...
<img src="F:\VS\jax-s_notebook\笔记图片\1749785145866.png" style="zoom:25%;" />

**常用的bounding volume方法`AABB`：**通常使用的方法是通过**三对和xyz轴平行的平面构成的长方体**来作为bounding volume；

<img src="F:\VS\jax-s_notebook\笔记图片\1749785330350.png" style="zoom:25%;" />

下面介绍如何判断**光线和AABB是否存在交点：**我们拿二维平面来举例，求交点的时候我们要**假设AABB的三对平面（当然这里举的例子只有x，y方向的两对平面）都是无限大的，**通过求解**光线到达这些无限大的平面的交点的时间来判断光线是否和盒子存在交点；**

求解的原则：**光线在三对平面之内才算在盒子内，光线只要离开一对平面，那么光线就离开了盒子；**基于此我们可以计算出光线**到达每对平面的时间，因为每对平面有两个时间，较短的记为tmin，较长的记为tmax，**之后将三对平面的tmin存在一个集合里面，三对平面的tmax存在另一个集合里面，那么**进入盒子的时间就是max(tmin)，因为只有光线均在在三对平面之内才算在盒子内，离开盒子的时间就是min(tmax)，那么如果进入盒子的时间小于离开盒子的时间，就证明光线和盒子有交点；**

<img src="F:\VS\jax-s_notebook\笔记图片\1749786303212.png" style="zoom:50%;" />

<img src="F:\VS\jax-s_notebook\笔记图片\1749787405171.png" style="zoom:50%;" />

但是我们忽略了一点，就是光线是一个射线，而不是直线，那么对于负值我们该如何考虑：

- 如果离开时间是负的，也就是三对平面的min(tmax)是负的，那就证明这个盒子一定在光源背后，所以肯定没有交点；
- 如果离开时间>=0，但是进入时间为负，那么光源一定在盒子内部，所以光线和盒子一定有交点；

**最后总结一下：当进入时间小于离开时间，并且离开时间>=0就一定有交点；**

那么为什么要用和坐标轴平行的面作为bounding volume的面呢？因为很容易计算是否相交，并且也很容易计算交点；
